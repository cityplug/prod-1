...
# Debian 8GB - (speedhole-prod-1) setup script - pve thincentre {101}
...
#> cd /opt && git clone https://github.com/cityplug/prod-1 && mv /opt/prod-1/speedhole-prod-1 /opt/speedhole-prod-1
#> chmod +x /opt/speedhole-prod-1/* && cd /opt/speedhole-prod-1 && ./run.sh
...
wget https://raw.githubusercontent.com/cityplug/prod-1/main/speedhole-prod-1/10-uname -O /etc/update-motd.d/10-uname && chmod +x /etc/update-motd.d/10-uname
...
cd /opt/speedhole-prod-1/ && docker-compose up -d && docker ps
